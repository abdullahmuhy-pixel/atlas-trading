...
// Place in SAME folder as atlas.html, index.html (landing) and signal-dashboard.html
// ================================================================
'use strict';
...
self.addEventListener('install', e => {
  e.waitUntil((async () => {
    ...
    await Promise.allSettled([
      fetch('./atlas.html', { cache: 'reload' }).then(r => r.ok && cache.put('./atlas.html', r)),
...
self.addEventListener('notificationclick', e => {
  ...
  e.waitUntil(
    clients.matchAll({ type: 'window', includeUncontrolled: true }).then(list => {
      // NOTE: root '/' is now the public landing page, not ATLAS itself —
      // only focus a tab that's actually ATLAS (atlas.html).
      for (const c of list) {
        if (c.url.includes('atlas.html')) return c.focus();
      }
      return clients.openWindow('./atlas.html');
    })
  );
});
